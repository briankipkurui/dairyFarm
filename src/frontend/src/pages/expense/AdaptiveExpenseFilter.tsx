...
const AdaptiveExpenseFilter: React.FC<AdaptiveValueChainFilterProps> = ({setData}) => {
    ...
    useEffect(() => {
        const fetchMetadata = async () => {
            try {
                const response = await fetch("/api/v1/expense/metadata");
                if (!response.ok) {
    ...
    const applyFilters = async () => {
        try {
            const response = await fetch("/api/v1/expense/filter", {
                method: "POST",
...
export default AdaptiveExpenseFilter;
